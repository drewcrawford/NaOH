...
func aliceBob() -> (CryptoBoxSecretKey, CryptoBoxSecretKey) {
    ...
    #else
        var alicePath = NSBundle(forClass: CarolineEngineTests.self).pathForResource("alice", ofType: "key")!
        var bobPath = NSBundle(forClass: CarolineEngineTests.self).pathForResource("bob", ofType: "key")!
    #endif
